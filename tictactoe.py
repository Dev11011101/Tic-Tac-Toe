...
#During the computers turn, it use the minimax algorithm to increase its chance of winning
#by checking the possible future states of the game and making the most best decisions for its moves
import pygame 
import math
import random
...
class Game_Window:
    ...
    def print_winner(self, letter, screen):
        ...
        font_style_win = pygame.font.Font("fonts.ttf", 24) 
        ...
        screen.blit(mesg, [xB+50, yB+376])
        replay_string = "Press R to restart"
        mesg_replay = font_style_win.render(replay_string, True, white)
        screen.blit(mesg_replay, [xB+220, yB+376])

    def print_letter(x_point, y_point, screen, letter):
        global board
        font_style_XO = pygame.font.Font("fonts.ttf", 65) 
...
def print_option(screen, color, xB, yB):
    font_style = pygame.font.Font("fonts.ttf", 35)
    font_style_sub = pygame.font.Font("fonts.ttf", 25)
    msg1 = "Press key X or O"
    mesg1 = font_style.render(msg1, True, color)
    screen.blit(mesg1, [xB+80, yB+150])
    msg2 = "To choose your letter"
    mesg2 = font_style.render(msg2, True, color)
    screen.blit(mesg2, [xB+50, yB+200])

def user_screen():
    global xB, yB, user_key, comp_key
    running = True
    screenDim = pygame.display.set_mode((400, 400), pygame.RESIZABLE)
    bmp = pygame.image.load('gameicon.bmp')
